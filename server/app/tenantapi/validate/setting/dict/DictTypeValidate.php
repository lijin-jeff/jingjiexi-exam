...
namespace app\tenantapi\validate\setting\dict;
...
/**
 * 字典类型验证
 * Class TenantDictTypeValidate
 * @package app\tenantapi\validate\setting\dict 
 */
class TenantDictTypeValidate extends BaseValidate
{
    
    protected $rule = [
        'id' => 'require|checkDictType',
        'name' => 'require|length:1,255',
        'type' => 'require|unique:' . TenantDictType::class,
    ...
    {
        $dictType = TenantDictType::findOrEmpty($value);
        if ($dictType->isEmpty()) {
            return '字典类型不存在';
        }
        return true;
    ...
    {
        $dictData = TenantDictData::whereIn('type_id', $value)->select();
