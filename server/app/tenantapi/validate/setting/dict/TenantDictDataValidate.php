...
/**
 * 字典数据验证
 * Class TenantDictDataValidate
 * @package app\tenantapi\validate\setting\dict
 */
class TenantDictDataValidate extends BaseValidate
{
    ...
    {
        $article = TenantDictData::findOrEmpty($value);
        if ($article->isEmpty()) {
            return '字典数据不存在';
        }
        return true;
    ...
    {
        $type = TenantDictType::findOrEmpty($value);
        if ($type->isEmpty()) {
            return '字典类型不存在';
        }
        return true;
