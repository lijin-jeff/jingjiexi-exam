...
function linear_to_tree_iterative($data, $sub_key_name = 'sub', $id_name = 'id', $parent_id_name = 'pid', $parent_id = 0)
{
    // 1. 构建哈希表：id => 节点（O(n) 时间）
    $id_map = [];
    foreach ($data as $row) {
        $id = $row[$id_name];
        $id_map[$id] = $row;
        // 初始化子节点数组（避免后续判断）
        $id_map[$id][$sub_key_name] = [];
    }

    // 2. 构建树形结构：先找根节点，再逐层挂子节点
    $tree = [];
    foreach ($id_map as $id => $node) {
        $pid = $node[$parent_id_name];
        if ($pid == $parent_id) {
            // 根节点：加入结果集
            $tree[] = &$id_map[$id];
        } else {
            // 子节点：挂到父节点的 sub 数组中（需确保父节点存在）
            if (isset($id_map[$pid])) {
                $id_map[$pid][$sub_key_name][] = &$id_map[$id];
            }
        }
    }
