...
class ArticleLists extends BaseApiDataLists implements ListsSearchInterface
{
    ...
    {
        $where[] = ['is_show', '=', 1];
        $where[] = ['exam_category_uid', '=', $this->params['exam_category_uid']];
    ...
    public function lists(): array
    {
        ...
        $field = 'id,cid,exam_category_uid,title,author,desc,image,click_virtual,click_actual,create_time,is_external_link,external_link';
        $result = Article::field($field)
            ->with(['examCategory'])
            ->where($this->queryWhere())
            ->where($this->searchWhere)
            ->orderRaw($orderRaw)
            ->append(['click', 'exam_category_title'])
