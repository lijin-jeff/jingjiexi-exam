...
class UserLogic extends BaseLogic
{
    ...
    public static function info(int $userId)
    {
        $user = User::where(['id' => $userId])
            ->field('id,sn,sex,account,password,nickname,real_name,avatar,mobile,create_time,user_money,remark,integral,vip_state,vip_endTime,is_admin,login_time,login_ip,auto_eliminate_threshold')
            ->findOrEmpty();
        $user['has_password'] = !empty($user['password']);
     ...
     */
    public static function updateEliminateThreshold(array $params)
    {
        try {
            User::update([
                'auto_eliminate_threshold' => $params['auto_eliminate_threshold'],
            ], ['id' => $params['user_id']]);
        } catch (\Exception $e) {
            self::setError($e->getMessage());
            return false;
