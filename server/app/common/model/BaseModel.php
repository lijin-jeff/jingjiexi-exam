    ...
    public static function scopeTenantId($query): void
    {
        ...
        if ($tenantId || ($hasTenantId && isset(\request()->tenantId))) {
            ...
            if (!in_array($tableNoPrefix, self::$notCheckTables)) {
                // 根据租户唯一标识获取对应表是否存在，使用静态缓存避免重复查询
                if (!isset($tenantCache[$tenantId])) {
                    $tenant = Tenant::query()->where('id', $tenantId)->find();
                    $tenantCache[$tenantId] = $tenant ? $tenant->toArray() : [];
                }
                $tenantInfo = $tenantCache[$tenantId];
                // 租户分表策略
                if (!empty($tenantInfo) && $tenantInfo['tactics'] == 1) {
