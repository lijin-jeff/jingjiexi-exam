...
abstract class BaseDataLists implements ListsInterface
{
    ...
    public int $pageType = 1; //默认类型：1-一般分页；0-不分页，获取最大所有数据
    ...
    public function __construct()
    {
        ...
        //请求参数设置
        $this->request = app(\app\Request::class);
    ...
    {
        $this->pageSizeMax = Config::get('project.lists.page_size_max', 1000);
        $this->pageSize = Config::get('project.lists.page_size', 15);
        $this->pageType = $this->request->get('page_type', 1);

        //print_r($this->pageType);
        if ($this->pageType == 1) {
            //分页
