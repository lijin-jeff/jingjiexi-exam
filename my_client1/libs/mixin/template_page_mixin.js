  ...
  methods: {
    ...
    async updateCustomBarInfo() {
      try {
        ...
        // 如果获取失败则重新获取
        if (!customBarHeight && this.$tn && typeof this.$tn.updateCustomBar === 'function') {
          try {
        ...
        // 更新vuex中的导航栏信息
        if (this.$tn && typeof this.$tn.vuex === 'function') {
    ...
    getSubscribeTemplates() {
      this.$api.getSubscribeTemplates().then(res => {
        if ((res.code === 1 || res.code === 100) && res.data && res.data.length > 0) {
          // 将订阅模板ID存储到Vuex
          this.$store.commit('$tStore', {
            name: 'vuex_subscribe_templates',
            value: res.data
          })
        }
      }).catch(() => {
