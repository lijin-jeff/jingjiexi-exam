...
const optionCheck = (check: string | number | boolean | undefined) => {
    formData.answer = [check as string]
}
...
const fetchQuestionDetail = async () => {
    try {
        const res = (await apiTenantExamQuestionDetail({ id: props.id })) as QuestionDetailRes
        // 检查 res 是否存在
        if (res) {
            // 合并数据，确保数值类型正确
            ...
            // 检查 res.option 是否存在且为数组
            if (res.option && Array.isArray(res.option)) {
                // 根据is_check状态构建选中数组
                formData.answer = res.option
                ...
                // 更新选项数据
                formData.option = res.option
            }

            // 处理知识点数据
            if (typeof res.knowledge_uid === 'string') {
                formData.knowledge_uid = res.knowledge_uid
                    .split(',')
                    .map((item: string) => item.trim())
            } else if (Array.isArray(res.knowledge_uid)) {
                formData.knowledge_uid = res.knowledge_uid
            } else {
            ...
            // 处理标签数据
            if (typeof res.label_uid === 'string') {
                formData.label_uid = res.label_uid.split(',').map((item: string) => item.trim())
            } else if (Array.isArray(res.label_uid)) {
                formData.label_uid = res.label_uid
            }
